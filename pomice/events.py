...
class NodeReadyEvent(PomiceEvent):
    """Fired when a node is connected to Lavalink.
    """
    name = "pomice_node_ready"

    def __init__(self, node: Node):
        self.node: Node = node

        # on_pomice_node_ready(node)
        self.handler_args = self.node,

    def __repr__(self) -> str:
        return f"<Pomice.NodeReadyEvent node={self.node!r}>"
